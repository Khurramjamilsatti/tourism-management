...
<div class="card mb-3">
    <div class="card-header bg-white">
        <h6 class="mb-0"><i class="bi bi-bar-chart"></i> Records by Budget Category</h6>
    </div>
    <div class="card-body">
        ...
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $budget->name }}</td>
                    <td><a href="{{ URL::to('tourism-data?budget=' . $budget->name) }}" class="badge rounded-pill bg-primary text-decoration-none">{{ $budget->tourism_data_count }}</a></td>
                    <td>
                        @if(auth()->user()->isAdmin())
...
<script>
    new Chart(document.getElementById('budgetChart'), {
        type: 'bar',
        data: {
            labels: {!! json_encode($budgetCategories->pluck('name')) !!},
            datasets: [{
                label: 'Tourism Records',
                data: {!! json_encode($budgetCategories->pluck('tourism_data_count')) !!},
