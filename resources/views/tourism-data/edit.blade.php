...
@section('title', 'Edit Tourism Data')
@section('page-title', 'Edit Tourism Data')
    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header bg-white">
                <h6 class="mb-0"><i class="bi bi-pencil"></i> Edit Tourism Data Entry</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('tourism-data.update', $tourismData) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="visitor_name" class="form-label fw-semibold">Visitor Name <small class="text-muted">(optional)</small></label>
                            <input type="text" class="form-control @error('visitor_name') is-invalid @enderror" id="visitor_name" name="visitor_name" value="{{ old('visitor_name', $tourismData->visitor_name) }}">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('country_id') is-invalid @enderror" id="country_id" name="country_id" required>
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ old('country_id', $tourismData->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('city_visited') is-invalid @enderror" id="city_visited" name="city_visited" value="{{ old('city_visited', $tourismData->city_visited) }}" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('purpose_id') is-invalid @enderror" id="purpose_id" name="purpose_id" required>
                                ...
                                @foreach($purposes as $purpose)
                                    <option value="{{ $purpose->id }}" {{ old('purpose_id', $tourismData->purpose_id) == $purpose->id ? 'selected' : '' }}>{{ $purpose->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('visit_date') is-invalid @enderror" id="visit_date" name="visit_date" value="{{ old('visit_date', $tourismData->visit_date->format('Y-m-d')) }}" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('month') is-invalid @enderror" id="month" name="month">
                                ...
                                @foreach(['January','February','March','April','May','June','July','August','September','October','November','December'] as $i => $m)
                                    <option value="{{ $i + 1 }}" {{ old('month', $tourismData->month) == ($i + 1) ? 'selected' : '' }}>{{ $m }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('age_group') is-invalid @enderror" id="age_group" name="age_group">
                                ...
                                @foreach($ageGroups as $group)
                                    <option value="{{ $group->name }}" {{ old('age_group', $tourismData->age_group) == $group->name ? 'selected' : '' }}>{{ $group->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('travel_type') is-invalid @enderror" id="travel_type" name="travel_type">
                                ...
                                @foreach($travelTypes as $type)
                                    <option value="{{ $type->name }}" {{ old('travel_type', $tourismData->travel_type) == $type->name ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('budget') is-invalid @enderror" id="budget" name="budget">
                                ...
                                @foreach($budgetCategories as $b)
                                    <option value="{{ $b->name }}" {{ old('budget', $tourismData->budget) == $b->name ? 'selected' : '' }}>{{ $b->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('duration') is-invalid @enderror" id="duration" name="duration" value="{{ old('duration', $tourismData->duration) }}" min="1" placeholder="Number of days">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('satisfaction') is-invalid @enderror" id="satisfaction" name="satisfaction">
                                ...
                                @for($s = 1; $s <= 5; $s++)
                                    <option value="{{ $s }}" {{ old('satisfaction', $tourismData->satisfaction) == $s ? 'selected' : '' }}>{{ $s }} - {{ ['Very Poor','Poor','Average','Good','Excellent'][$s-1] }}</option>
                                @endfor
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control @error('previous_visits') is-invalid @enderror" id="previous_visits" name="previous_visits" value="{{ old('previous_visits', $tourismData->previous_visits) }}" min="0" placeholder="0">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('spending') is-invalid @enderror" id="spending" name="spending" value="{{ old('spending', $tourismData->spending) }}" min="0" placeholder="0.00">
                            @error('spending')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4 mb-3 d-flex align-items-end">
                            <div class="form-check">
                                <input class="form-check-input @error('will_return') is-invalid @enderror" type="checkbox" id="will_return" name="will_return" value="1" {{ old('will_return', $tourismData->will_return) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        <label for="feedback" class="form-label fw-semibold">Feedback/Comments</label>
                        <textarea class="form-control @error('feedback') is-invalid @enderror" id="feedback" name="feedback" rows="3">{{ old('feedback', $tourismData->feedback) }}</textarea>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-circle"></i> Update Record</button>
